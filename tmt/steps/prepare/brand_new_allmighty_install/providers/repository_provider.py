...
class RepositoryArtifactInfo(ArtifactInfo):
    ...
    def __init__(self, url: str, **kwargs: Any) -> None:
        ...
        """
        super().__init__(id=0, _raw_artifact={}, **kwargs)  # Dummy id and empty _raw_artifact
        self.url = url
    ...
    def name(self) -> str:
        ...
        :raises ValueError: If the URL does not point to a .repo file
        """
        parsed_url = urlparse(self.url)
        filename = Path(parsed_url.path).name
        if not filename.endswith('.repo'):
            raise ValueError(f"URL must point to a .repo file, got: {filename}")
        if not filename.strip():  # Check for empty or malformed filename
...
class RepositoryProvider(ArtifactProvider[RepositoryArtifactInfo]):
    ...
    def _parse_artifact_id(self, artifact_id: str) -> str:
        ...
        if not artifact_id.startswith(('http://', 'https://')):
            raise ValueError(f"Invalid repository URL: {artifact_id}")
    ...
    def list_artifacts(self) -> Iterator[RepositoryArtifactInfo]:
        ...
        yield RepositoryArtifactInfo(url=self.artifact_id)
    ...
    def _download_artifact(
        ...
        try:
            # Ensure temporary download directory exists on guest
            guest.execute(
                ShellScript(
                    f"[ -d {quote(str(destination.parent))} ] || "
                    f'{"sudo " if not guest.facts.is_superuser else ""}'
                    f"mkdir -p {quote(str(destination.parent))}"
                ).to_shell_command(),
                silent=True,
            )
            ...
            guest.execute(
                ShellScript(
                    f"curl -L --fail -o {quote(str(destination))} {quote(url)}"
                ).to_shell_command(),
                silent=True,
            )
            ...
            guest.execute(
                ShellScript(
                    f'{"sudo " if not guest.facts.is_superuser else ""}'
                    f"mv {quote(str(destination))} {quote(str(repo_destination))} && "
                    f'{"sudo " if not guest.facts.is_superuser else ""}'
                    f"chmod 644 {quote(str(repo_destination))}"
                ).to_shell_command(),
                silent=True,
            )
            ...
            raise DownloadError(f"Failed to process {url} to {repo_destination}: {e!s}")
